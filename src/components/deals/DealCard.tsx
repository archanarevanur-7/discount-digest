...
import { Lock, ExternalLink, Zap, Clock, AlertCircle } from "lucide-react";
...
export default function DealCard({ deal, isLocked }: DealCardProps) {
  const locked = isLocked ?? deal.isLocked;
  ...
  return (
    <div className="relative group">
      ...
      {/* Locked overlay */}
      {locked && (
        <div className="absolute inset-0 rounded-xl backdrop-blur-sm bg-white/70 flex flex-col items-center justify-center gap-3 p-4">
          <div className="bg-zinc-900 text-white rounded-full p-3">
            <Lock className="h-5 w-5" />
          </div>
          <p className="font-semibold text-zinc-900 text-center text-sm">
            Unlock with your .edu
          </p>
          <p className="text-xs text-zinc-500 text-center">
            Enter your student email to see this deal and 10 more
          </p>
          <Button size="sm" asChild>
            <a href="/#email-capture">Unlock free →</a>
          </Button>
        </div>
      )}
    </div>
